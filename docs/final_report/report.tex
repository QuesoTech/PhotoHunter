...
One of the largest roadblocks to successful computer vision is the difficulty
of acquiring large quantities of accurate ground truth data for a given problem
statement. Traditionally, this task has been relegated to under-paid and
over-worked underclassmen or new graduate students. This technique has produced
some of the most famous datasets in the field (Caltech-101, Caltech-256).
However, the size of these datasets no longer supports current machine learning
research. Recently, many researchers have opted to use Amazon Mechanical
Turk~\footnote{\url{https://www.mturk.com/}} to create and verify their
...
\subsection{Issues}
We encountered small issues throughout the semester.  Integrating the mobile
applications with the Facebook API proved to be challenging.  We used a
Facebook plugin for Cordova to allow logging in through Facebook accounts, but
it took many different sets of documentation and trial and error to figure out
how to work with the Facebook API.  Integration with PostGIS on the web backend
caused problems as well.  Issues with the PostGreSQL driver and Go caused us to
have to search for work arounds for inserting geo data.
...
The main limitation encountered during the semester was time.  Missed time due 
to weather and work for other classes/jobs led to a loss in time to work on 
the project.  Testing the applications and interface with large amounts of 
data was another limitation.  Four people can submit and label only so many
images, and having a mass of users flood the servers was something we couldn't
simulate.
